...
## Bước 1: Cập nhật `MangaForm.jsx` để xử lý giá trị khởi tạo cho các trường select

Cập nhật file `MangaReader_ManagerUI\mangareader_managerui.client\src\features\manga\components\MangaForm.jsx` để đảm bảo các giá trị mặc định và giá trị từ `initialData` được gán đúng cách cho các trường select, tránh giá trị `undefined`.
...
function MangaForm({ initialData, onSubmit, isEditMode }) {
  const {
    ...
    defaultValues: { // Giá trị mặc định ban đầu cho TẤT CẢ các trường
        title: '',
        originalLanguage: 'ja',
  ...
  useEffect(() => {
    if (isEditMode && initialData) {
      reset({
        ...
        // Không cần reset tempAuthor, tempAuthorRole ở đây vì chúng là trường tạm thời
      });

      // Xử lý selectedAuthorsVisual và selectedTagsVisual
      ...
      // Reset cho chế độ tạo mới nếu cần, dù defaultValues đã xử lý
      reset({
        title: '',
        originalLanguage: 'ja',
        publicationDemographic: null,
      ...
      <Grid container spacing={2} columns={{ xs: 4, sm: 6, md: 12 }}>
        ...
        <Grid
          ...
          <FormInput
            ...
            // Thêm một MenuItem rỗng nếu giá trị có thể là null và muốn hiển thị "Không chọn"
            // Hoặc đảm bảo FormInput/Select xử lý value={null} đúng cách
            // Ví dụ:
            // renderValue={(value) => value === null ? <em>Không chọn</em> : options.find(opt => opt.value === value)?.label}
          />
        </Grid>
        <Grid
          size={{
        ...
        {isEditMode && (
          <Grid
            ...
            <FormControlLabel
              control={
                <Switch
                  checked={!!isLocked} // Đảm bảo isLocked là boolean
...
## Bước 2: Cập nhật `FormInput.jsx` để xử lý giá trị `null` cho `Select` (tùy chọn, nếu Bước 1 chưa đủ)

Nếu sau Bước 1 lỗi vẫn còn, bạn có thể thử điều chỉnh `FormInput.jsx` để `Select` của MUI nhận `value=""` khi giá trị thực tế là `null`. Lưu ý rằng điều này có thể ảnh hưởng đến giá trị được submit nếu schema Zod của bạn mong đợi `null` thay vì `""`.

```javascript
// MangaReader_ManagerUI\mangareader_managerui.client\src\components\common\FormInput.jsx
...
function FormInput({ name, control, label, type = 'text', options, ...props }) {
  ...
  if (type === 'select' && options) {
    // Xử lý giá trị null: Nếu field.value là null, truyền chuỗi rỗng cho Select.
    // Điều này giúp MUI Select không báo lỗi "out-of-range" khi giá trị là null 
    // và không có MenuItem nào có value={null}.
    // Lưu ý: Nếu schema của bạn mong đợi null và bạn có MenuItem value={null} thì không cần dòng này.
    ...
    return (
      <FormControl fullWidth margin="normal" error={!!error}>
        ...
        <Select
          labelId={`${name}-label`}
          id={name}
          {...field} // Truyền field gốc
          value={selectValue} // Sử dụng giá trị đã được điều chỉnh cho Select của MUI
          label={label}
          {...props}
        >
          {/* Optional: Thêm MenuItem cho giá trị rỗng nếu trường đó thực sự có thể rỗng 
              và bạn muốn người dùng có thể chọn "Không chọn" / "Để trống"
              Ví dụ cho publicationDemographic (nếu giá trị rỗng đại diện cho null):
          */}
          { (name === 'publicationDemographic' || name === 'year') && ( /* Chỉ áp dụng cho các trường cho phép null */
            <MenuItem value="">
